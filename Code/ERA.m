...
function [Result]=ERA(Y,fs,ncols,nrows,inputs,cut,shift,EMAC_option)
    ...
    fd=(imag(s));           % damped natural freqs:
    ...
    [C1,II] = max(abs(Phi));
        for jj = 1:HH(2)
            ...
            ModeShapeS(:,jj) = (Phi(:,jj)*exp(1i*b));
            ModeShapeS(:,jj) = Phi(:,jj)/norm(Phi(:,jj));
        end
    ...
    %   Output format    *
    %*********************
    NaFreq=fd;             % Natural frequencies vector
    DampRatio=zeta;        % Damping ratios vector
    shapes=ModeShapeS;       % Mode shape matrix
    %Froming Result structure
    %------------------------
    Result.Parameters.Poles = s;
