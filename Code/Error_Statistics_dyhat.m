function [Error_Stats] = Error_Statistics_dyhat(Data_Structure,seed_instances,snr_instances,Parameter_Average,Average_Variable)
    % --------- Inputs -------------
    ...
    switch Average_Variable
        case 'Modes'
            ...
            ephi_gcvd_dyhat_mat = Data_Structure.gcvd_dyhat; % 
        ...
        case 'MPS'
            ...
            mps_gcvd_dyhat_mat = Data_Structure.gcvd_dyhat;
            mps_gcvd_dyhat_corrected_mat = Data_Structure.gcvd_dyhat_corrected;
            mps_gcvd_dyhat_r_mat = Data_Structure.gcvd_dyhat_r;
        ...
        case 'MPS_Error'
            ...
            mps_gcvd_dyhat_err_mat = Data_Structure.gcvd_dyhat;
            mps_gcvd_dyhat_corrected_err_mat = Data_Structure.gcvd_dyhat_corrected;
            mps_gcvd_dyhat_r_err_mat = Data_Structure.gcvd_dyhat_r;
        ...
        case 'MAC'
            ...
            mac_gcvd_dyhat_mat = Data_Structure.gcvd_dyhat;
    ...
    switch Average_Variable
        case 'Modes'
            if isnumeric(Parameter_Average)
                ...
                % GCVD-dyhat
                ephi_gcvd_dyhat_stat(:,1,:) = median(ephi_gcvd_dyhat_mat, Parameter_Average);
                ephi_gcvd_dyhat_stat(:,2,:) = std(ephi_gcvd_dyhat_mat, [], Parameter_Average);
                ephi_gcvd_dyhat_stat(:,3,:) = min(ephi_gcvd_dyhat_mat, [], Parameter_Average);
                % DMD
                ...
                switch Parameter_Average
                    case 'Seed'
                        ...
                        ephi_gcvd_dyhat_stat = zeros(n,3,snr_instances);
                        ...
                        % GCVD-dyhat
                        ephi_gcvd_dyhat_stat(:,1,:) = median(ephi_gcvd_dyhat_mat,2);
                        ephi_gcvd_dyhat_stat(:,2,:) = std(ephi_gcvd_dyhat_mat,[],2);
                        ephi_gcvd_dyhat_stat(:,3,:) = min(ephi_gcvd_dyhat_mat,[],2);
                        % DMD
                    ...
                    case 'SNR'
                        ...
                        ephi_gcvd_dyhat_stat = zeros(n,3,seed_instances);
                        ...
                        % GCVD-dyhat
                        ephi_gcvd_dyhat_stat(:,1,:) = median(ephi_gcvd_dyhat_mat,3);
                        ephi_gcvd_dyhat_stat(:,2,:) = std(ephi_gcvd_dyhat_mat,[],3);
                        ephi_gcvd_dyhat_stat(:,3,:) = min(ephi_gcvd_dyhat_mat,[],3);
                        % DMD
            ...
            Error_Stats.gcvd_dyhat = ephi_gcvd_dyhat_stat;
        ...
        case 'MPS'
            switch Parameter_Average
                case 'Seed'
                    ...
                    mps_gcvd_dyhat_stat = zeros(3,n,3,snr_instances); % 
                    mps_gcvd_dyhat_corrected_stat = zeros(3,n,3,snr_instances); %
                    mps_gcvd_dyhat_r_stat = zeros(3,n,3,snr_instances); %
                    ...
                    % GCVD-dyhat
                    mps_gcvd_dyhat_stat(:,:,1,:) = median(mps_gcvd_mat,3);
                    mps_gcvd_dyhat_stat(:,:,2,:) = std(mps_gcvd_mat,[],3);
                    mps_gcvd_dyhat_stat(:,:,3,:) = min(mps_gcvd_mat,[],3);
                    % GCVD-dyhat-corrected
                    mps_gcvd_dyhat_corrected_stat(:,:,1,:) = median(mps_gcvd_mat,3);
                    mps_gcvd_dyhat_corrected_stat(:,:,2,:) = std(mps_gcvd_mat,[],3);
                    mps_gcvd_dyhat_corrected_stat(:,:,3,:) = min(mps_gcvd_mat,[],3);
                    % GCVD-dyhat-r
                    mps_gcvd_dyhat_r_stat(:,:,1,:) = median(mps_gcvd_mat,3);
                    mps_gcvd_dyhat_r_stat(:,:,2,:) = std(mps_gcvd_mat,[],3);
                    mps_gcvd_dyhat_r_stat(:,:,3,:) = min(mps_gcvd_mat,[],3);
                    % DMD
                ...
                case 'SNR'
                    ...
                    mps_gcvd_dyhat_stat = zeros(3,n,3,seed_instances);
                    mps_gcvd_dyhat_corrected_stat = zeros(3,n,3,seed_instances);
                    mps_gcvd_dyhat_r_stat = zeros(3,n,3,seed_instances);
                    ...
                    % GCVD-dyhat
                    mps_gcvd_dyhat_stat(:,:,1,:) = median(mps_gcvd_dyhat_mat,4);
                    mps_gcvd_dyhat_stat(:,:,2,:) = std(mps_gcvd_dyhat_mat,[],4);
                    mps_gcvd_dyhat_stat(:,:,3,:) = min(mps_gcvd_dyhat_mat,[],4);
                    % GCVD-dyhat-corrected
                    mps_gcvd_dyhat_corrected_stat(:,:,1,:) = median(mps_gcvd_dyhat_corrected_mat,4);
                    mps_gcvd_dyhat_corrected_stat(:,:,2,:) = std(mps_gcvd_dyhat_corrected_mat,[],4);
                    mps_gcvd_dyhat_corrected_stat(:,:,3,:) = min(mps_gcvd_dyhat_corrected_mat,[],4);
                    % GCVD-dyhat-r
                    mps_gcvd_dyhat_r_stat(:,:,1,:) = median(mps_gcvd_dyhat_r_mat,4);
                    mps_gcvd_dyhat_r_stat(:,:,2,:) = std(mps_gcvd_dyhat_r_mat,[],4);
                    mps_gcvd_dyhat_r_stat(:,:,3,:) = min(mps_gcvd_dyhat_r_mat,[],4);
                    % DMD
            ...
            Error_Stats.gcvd_dyhat = mps_gcvd_stat;
            Error_Stats.gcvd_dyhat_corrected = mps_gcvd_dyhat_corrected_stat;
            Error_Stats.gcvd_dyhat_r = mps_gcvd_dyhat_r_stat;
        ...
        case 'MPS_Error'
            if isnumeric(Parameter_Average)
                ...
                % GCVD-dyhat
                mps_gcvd_dyhat_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_err_mat, Parameter_Average);
                mps_gcvd_dyhat_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_err_mat,[], Parameter_Average);
                mps_gcvd_dyhat_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_err_mat,[], Parameter_Average);
                % GCVD-dyhat-corrected
                mps_gcvd_dyhat_corrected_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_corrected_err_mat, Parameter_Average);
                mps_gcvd_dyhat_corrected_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_corrected_err_mat,[], Parameter_Average);
                mps_gcvd_dyhat_corrected_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_corrected_err_mat,[], Parameter_Average);
                % GCVD-dyhat-r
                mps_gcvd_dyhat_r_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_r_err_mat, Parameter_Average);
                mps_gcvd_dyhat_r_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_r_err_mat,[], Parameter_Average);
                mps_gcvd_dyhat_r_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_r_err_mat,[], Parameter_Average);
                % DMD
                ...
                switch Parameter_Average
                    case 'Seed'
                        ...
                        % GCVD-dyhat
                        mps_gcvd_dyhat_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_err_mat,3);
                        mps_gcvd_dyhat_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_err_mat,[],3);
                        mps_gcvd_dyhat_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_err_mat,[],3);
                        % GCVD-dyhat-corrected
                        mps_gcvd_dyhat_corrected_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_corrected_err_mat,3);
                        mps_gcvd_dyhat_corrected_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_corrected_err_mat,[],3);
                        mps_gcvd_dyhat_corrected_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_corrected_err_mat,[],3);
                        % GCVD-dyhat-r
                        mps_gcvd_dyhat_r_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_r_err_mat,3);
                        mps_gcvd_dyhat_r_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_r_err_mat,[],3);
                        mps_gcvd_dyhat_r_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_r_err_mat,[],3);
                    ...
                    case 'SNR'
                        ...
                        mps_gcvd_dyhat_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_err_mat,4);
                        mps_gcvd_dyhat_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_err_mat,[],4);
                        mps_gcvd_dyhat_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_err_mat,[],4);

                        mps_gcvd_dyhat_corrected_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_corrected_err_mat,4);
                        mps_gcvd_dyhat_corrected_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_corrected_err_mat,[],4);
                        mps_gcvd_dyhat_corrected_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_corrected_err_mat,[],4);

                        mps_gcvd_dyhat_r_err_stat(:,:,1,:) = median(mps_gcvd_dyhat_r_err_mat,4);
                        mps_gcvd_dyhat_r_err_stat(:,:,2,:) = std(mps_gcvd_dyhat_r_err_mat,[],4);
                        mps_gcvd_dyhat_r_err_stat(:,:,3,:) = min(mps_gcvd_dyhat_r_err_mat,[],4);
            ...
            Error_Stats.gcvd_dyhat = mps_gcvd_dyhat_err_stat;
            Error_Stats.gcvd_dyhat_corrected = mps_gcvd_dyhat_corrected_err_stat;
            Error_Stats.gcvd_dyhat_r = mps_gcvd_dyhat_r_err_stat;
        ...
        case 'MAC'
            if isnumeric(Parameter_Average)
                ...
                mac_gcvd_dyhat_stat(:,:,1,:) = median(mac_gcvd_dyhat_mat,Parameter_Average);
                mac_gcvd_dyhat_stat(:,:,2,:) = std(mac_gcvd_dyhat_mat,[],Parameter_Average);
                mac_gcvd_dyhat_stat(:,:,3,:) = min(mac_gcvd_dyhat_mat,[],Parameter_Average);
                ...
                switch Parameter_Average
                    case 'Seed'
                        ...
                        mac_gcvd_dyhat_stat(:,:,1,:) = median(mac_gcvd_dyhat_mat,3);
                        mac_gcvd_dyhat_stat(:,:,2,:) = std(mac_gcvd_dyhat_mat,[],3);
                        mac_gcvd_dyhat_stat(:,:,3,:) = min(mac_gcvd_dyhat_mat,[],3);
                    ...
                    case 'SNR'
                        ...
                        mac_gcvd_dyhat_stat(:,:,1,:) = median(mac_gcvd_dyhat_mat,4);
                        mac_gcvd_dyhat_stat(:,:,2,:) = std(mac_gcvd_dyhat_mat,[],4);
                        mac_gcvd_dyhat_stat(:,:,3,:) = min(mac_gcvd_dyhat_mat,[],4);
            ...
            Error_Stats.gcvd_dyhat = mac_gcvd_dyhat_stat;
