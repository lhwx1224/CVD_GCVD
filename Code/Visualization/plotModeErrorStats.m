function plotModeErrorStats(ephis, markers, colors, alphas, legends, ephistd)
...
for i = 1:N
    ephi_i = ephis{i}; % Assign the modal assurance criteria 
    p1(i) = plot(ephi_i,markers{i},'Color',colors{i},'markersize',5,'linewidth',1.5);
    hold on
...
for i = 1:N
    ephi_i = ephis{i}; % Assign the modal assurance criteria 
    ephi_std_i = ephistd{i};
    [m_,n_] = size(ephi_i);
    x = 1:1:m_;
    p1(i) = errorbar(x,ephi_i,ephi_std_i,'Color',colors{i},'linewidth',1.5);
    hold on
    set([p1(i).Bar, p1(i).Line], 'Colortype','truecoloralpha', 'ColorData', [p1(i).Line.ColorData(1:3); 255*alphas(i)])
    s1(i) = scatter(x, ephi_i, 20, hex2rgb(colors(i)), 'Marker',markers{i}, 'LineWidth',1.5, 'MarkerEdgeAlpha',alphas(i));
end    
end
