function plotFreqErrorStat(mps_stat_mean, markers, colors, alphas, legends, mps_stat_std)
...
for i = 1:N
    ...
    p1(i) = plot(mps_hat_i(2,:),markers{i},'Color',colors{i},'markersize',5,'linewidth',1.5);
    hold on
...
for i = 1:N
    ...
    p1(i) = errorbar(x, mps_hat_i(2,:), mpsstd_i(2,:), ...
        ...
        'Color',colors{i}, ...
        'linewidth',1.5);
    hold on
    ...
    s1(i) = scatter(x, mps_hat_i(2,:), 20, hex2rgb(colors(i)), 'Marker',markers{i}, 'LineWidth',1.5, 'MarkerEdgeAlpha',alphas(i));
end    
end
% print('-dpng','-r600',strjoin({FigurePath,['Cmodes_',num2str(n),'dof_mps_err_comp_s-vd_gcvd.png']},filesep))
leg = legend([s1],legends,'Location','Northwest','interpreter','latex','numcolumns',3, 'location', 'Southwest','Fontsize',DefaultFontSize-1);
